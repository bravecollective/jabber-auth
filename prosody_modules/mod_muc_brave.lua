...
-- Might contain some stuff from mod_muc.lua, so see it's copyright for more details.
...
function muc_check_for_access(event)
    if event.stanza.attr.type == "unavailable" then
        return;
    end
    local to = jid.bare(event.stanza.attr.to);
    local from = jid.bare(event.stanza.attr.from);
    local from_node, from_host, from_resource = jid_split(event.stanza.attr.from);
    if not query('muc_access:'..from_host..":"..from_node..":"..to) then
...
module:hook("pre-presence/full", muc_check_for_access)
module:hook("presence/full", muc_check_for_access)
...
local host_room = muc_new_room(host);
host_room.route_stanza = room_route_stanza;
host_room.save = room_save;

local function get_disco_info(stanza)
    return st.iq({type='result', id=stanza.attr.id, from=host, to=stanza.attr.from}):query("http://jabber.org/protocol/disco#info")
...
local function get_disco_items(stanza)
    local reply = st.iq({type='result', id=stanza.attr.id, from=host, to=stanza.attr.from}):query("http://jabber.org/protocol/disco#items");
