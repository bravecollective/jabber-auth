...
local log = require "util.logger".init("mod_muc_brave_lib");
			...
			if current_nick then
				--if #pr == #stanza or current_nick ~= to then -- commented because google keeps resending directed presence
					if current_nick == to then -- simple presence
						...
						-- Refuse all nickname changes
						if self._occupants[to] or is_multisession or true then
							log("debug", "%s couldn't change nick", current_nick);
							local reply = st.error_reply(stanza, "cancel", "forbidden"):up();
			    ...
			    local tonode, tohost, toresource = jid_split(to);
			    local fromnode, fromhost, fromresource = jid_split(from);
			    local response = str_query("muc_nick:"..fromhost..":"..fromnode..":"..self:get_name())
			    local new_nick;
			    if response then
			        new_nick = response
			    else
			    ...
			    proper = tonode.."@"..tohost.."/"..new_nick;
			    local wrong = false;
			    if proper ~= to then
			        wrong = true;
			        --TODO: if the user has connected using a "groupchat 1.0" client (as indicated on joining the room by the lack of the MUC extension), then the service SHOULD deny the nickname change request and return a presence stanza of type "error" with a <not-acceptable/> error condition:
				    to = proper;
				end
				local is_merge;
				...
				elseif not new_nick then
					...
					-- BRAVE TODO: Send query to Core to update perms
					...
					local role = self:get_role(from);
	                if not role then	
	                    role = self:get_default_role(affiliation);
	                end;
...
function room_mt:handle_command(stanza)
    ...
    if command == "!kick" then
        ...
        local x = st.stanza("x", {xmlns = "http://jabber.org/protocol/muc#user"})
			...
			:up();
        local occupant = self._occupants[kickee_jid];
        presence_type = "unavailable";
        self._occupants[kickee_jid] = nil;
	...
	for jid,pres in pairs(occupant.sessions) do -- send to all sessions of the nick
        ...
        if bp then
		self:broadcast_except_nick(bp, kickee_jid);
	end
	return true;

    end
    if command == "!mad" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Server"}):tag("body"):text("http://boards.420chan.org/vg/src/1410756011700.gif"):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!wah" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Server"}):tag("body"):text("http://i.imgur.com/xugGpTu.gif"):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!dafuck" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Server"}):tag("body"):text("http://i.imgur.com/yDLBSWb.gif"):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!ref" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Server"}):tag("body"):text("http://i.imgur.com/XS5LK.gif"):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!wat" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Server"}):tag("body"):text("http://i.imgur.com/EiUJ5DS.jpg"):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!popcorn" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Server"}):tag("body"):text("https://i.imgur.com/W2eR7TM.gif"):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!tackled" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/DurrHurrDurr Bot [TEST]"}):tag("body"):text("https://soundcloud.com/townsaver/dhdiplo"):up()
        self:broadcast_message(s, false);
        return true;
    end
    ...
    if command == "!damn" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Server"}):tag("body"):text("http://www.scrapsyard.com/graphics/attitude/attitude01.gif"):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!test" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Server"}):tag("body"):text("http://i.imgur.com/e3zu8aj.png"):up()
        self:broadcast_message(s, false);
        return true;
    end
    ...
    if command == "!dhd" then
        ...
        while index < length do
            ...
            name = name .. segments[index];
            index = index + 1;
        end
        if name ~= "" then
            name = name .. ": ";
        end
        if (resource:sub(0, 11) ~= "Acid Katelo" and resource:sub(0, 12) ~= "DurrHurrDurr") then
            return true;
        end
        
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/DurrHurrDurr Bot [TEST]"}):tag("body"):text(name.."Kill yourself"):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!linus" then
        local node, host, resource;
        node, host, resource = jid_split(stanza.attr.from);
        if (resource:sub(0, 11) ~= "Acid Katelo" and resource:sub(0, 13) ~= "Lquid Drisseg") then
            return true;
        end
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Linus Torvalds Bot"}):tag("body"):text("\'We\'re talking \"sloth that was dropped on the head as a baby\" level retardation levels here\'"):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!resource" then
        local node, host, resource;
        node, host, resource = jid_split(stanza.attr.from);
        resource = resource:sub(segments[1] or 0, segments[2]);
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Server"}):tag("body"):text(resource):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!grath" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Grath Telkin Bot [-10.0]"}):tag("body"):text("https://soundcloud.com/nan-6/grath-rage-07-07-2013"):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!bl" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Anna niedostepny Bot [BRAVE]"}):tag("body"):text("Hey oh was just in a convo with are"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Elo Knight Bot [MEN.]"}):tag("body"):text("cool"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/are34 Bot [TISHU]"}):tag("body"):text("o hai"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Elo Knight Bot [MEN.]"}):tag("body"):text("hey real quick"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Anna niedostepny Bot [BRAVE]"}):tag("body"):text("invited him cause too many convos"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Elo Knight Bot [MEN.]"}):tag("body"):text("are you a girl"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Anna niedostepny Bot [BRAVE]"}):tag("body"):text("LOL"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/are34 Bot [TISHU]"}):tag("body"):text("hes a girl"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Anna niedostepny Bot [BRAVE]"}):tag("body"):text("Am i cute ?"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Elo Knight Bot [MEN.]"}):tag("body"):text("um"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Elo Knight Bot [MEN.]"}):tag("body"):text("i dunno how to say this"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/are34 Bot [TISHU]"}):tag("body"):text("i would bang anna"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Elo Knight Bot [MEN.]"}):tag("body"):text("but"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/are34 Bot [TISHU]"}):tag("body"):text("for real"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Elo Knight Bot [MEN.]"}):tag("body"):text("if I was making"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Elo Knight Bot [MEN.]"}):tag("body"):text("a cutie pie"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Elo Knight Bot [MEN.]"}):tag("body"):text("youd be the main ingredient"):up()
        self:broadcast_message(s, false);
        s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Vadeim Rizen Bot [MEN.]"}):tag("body"):text("elo prefers the company of men"):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!porque" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Server"}):tag("body"):text("https://i.imgur.com/BV8zk44.jpg"):up()
        self:broadcast_message(s, false);
        return true;
    end
    if command == "!well" then
        local s = st.message({type="groupchat", to=stanza.attr.to, from=jid_bare(stanza.attr.from).."/Server"}):tag("body"):text("http://i.imgur.com/uIwdrcJ.gif"):up()
        self:broadcast_message(s, false);
        return true;
    end
...
function room_mt:handle_to_room(origin, stanza) -- presence changes and groupchat messages, along with disco/etc
	log("info", "IN HANDLE_TO_ROOM")
	log("info", stanza.name)
	local type = stanza.attr.type;
	local xmlns = stanza.tags[1] and stanza.tags[1].attr.xmlns;
	log("warn", tostring(xmlns))
	if stanza.name == "iq" then
		...
		elseif xmlns == "http://jabber.org/protocol/muc#admin" then
			...
			local role = self:get_role(actor);
	        if not role then	
	            role = self:get_default_role(affiliation);
	        end;
		...
		elseif occupant.role == "visitor" then
			...
			if subject then
				...
				self:broadcast_message(stanza, self:get_historylength() > 0 and stanza:get_child("body"));
                                log("info", tostring(self._occupants));
                                --log("info", tostring("TEST LOGGING PLEASE IGNORE"));
                                self:handle_command(stanza);
			end
			stanza.attr.from = from;
		end
...
function room_mt:get_affiliation(jid)
	...
	local resp = str_query("muc_roles:"..host..":"..node..":"..self:get_name());
    local role;
    local aff;
    local ind = 0;
    for info in string.gmatch(resp, "[a-zA-Z_']+") do
        if ind == 0 then
            aff = info;
        end
        if ind == 1 then
...
function room_mt:can_set_role(actor_jid, occupant_jid, role)
	...
	if actor_jid == true then return true; end
        ...
        log("warn", tostring(actor_jid))
        log("warn", tostring(self._jid_nick[actor_jid]))
        log("warn", tostring(self._occupants[self._jid_nick[actor_jid]]))
        log("warn", tostring(self._occupants[actor_jid]))

	local actor = self._occupants[self._jid_nick[actor_jid]];
        if not actor then
            actor = self._occupants[actor_jid];
        end

        log("warn", tostring(actor.role))
        log("warn", tostring(occupant.affiliation))
        log("warn", tostring(occupant.role))
